...
export default function Home() {
  ...
  function wrongSignRationaleForChoice(choice: string, step: 1 | 2 | 3) {
    const selected = SIGN_SETS.find((sign) => signAnswerForStep(sign, step) === choice);
    if (!selected) return "Review the selected option and compare its usual clinical pattern with the clue in this question.";
    if (step === 1) return `${selected.sign} is associated with ${selected.condition}. Its key clue is ${selected.description} This does not match the finding described in this question.`;
    if (step === 2) return `${selected.condition} is classically linked with ${selected.sign}. Look for ${selected.description} This is why the selected condition does not fit this sign.`;
    return `That clinical meaning belongs with ${selected.sign}, which is associated with ${selected.condition}. It does not fit the sign in this question.`;
  }
        ...
        {view === "signs" && (
          <div className="page-content signs-page">
            ...
            <section className="panel signs-card">
              <div className="sign-card-head"><div><span className="section-kicker">{currentSign.category.toUpperCase()}</span><h3>Three-step recall</h3></div><span className="sign-step-chip">STEP {signStep} / 3</span></div>
              <div className="sign-recall-steps">{([1, 2, 3] as const).map((step) => { const prompt = signPromptForStep(currentSign, step); const complete = step < signStep; const active = step === signStep; return <section key={step} className={`sign-recall-step ${complete ? "complete" : active ? "active" : "upcoming"}`}><div className="sign-step-title"><span>STEP {step}</span><strong>{step === 1 ? "Name the sign" : step === 2 ? "Link the association" : "Explain why it matters"}</strong>{complete && <CheckCircle2 size={17} />}</div>{complete ? <p className="sign-complete-answer">{signAnswerForStep(currentSign, step)}</p> : active ? <><div className="sign-cue"><span>{prompt.label}</span><p>{prompt.cue}</p></div><h4 className="sign-question">{prompt.question}</h4><div className="sign-options" role="radiogroup" aria-label="Sign recall answers">{signOptions.map((option) => <button key={option} className={signChoice === option ? "selected" : ""} disabled={Boolean(signResult)} onClick={() => setSignChoice(option)} role="radio" aria-checked={signChoice === option}>{option}</button>)}</div><div className="sign-actions"><button className="secondary-button" onClick={() => resetSign()}><RotateCcw size={16} /> Restart sign</button><button className="primary-button" disabled={!signChoice || Boolean(signResult)} onClick={checkSignAnswer}><Check size={17} /> Check answer</button></div>{signResult && <div className={`sign-feedback ${signResult}`}><div>{signResult === "correct" ? <CheckCircle2 size={19} /> : <XCircle size={19} />}</div><div><strong>{signResult === "correct" ? signReviewQueue.includes(currentSign.id) ? "Complete - this sign is cleared from review." : "Complete - review it again separately if you missed it earlier." : "Not quite. Review the option you chose."}</strong><p>{signResult === "correct" ? currentSign.meaning : wrongSignRationaleForChoice(signChoice, signStep)}</p><button onClick={continueSignRecall}>{signResult === "wrong" ? "Try again" : "Next sign"} <ArrowRight size={15} /></button></div></div>}</> : <p className="sign-upcoming-copy">This step opens right here after you complete the step above.</p>}</section>; })}</div>
            </section>
